            ...
            while iTimes < t:
                ...
                if "redefines" not in obj[k]:
                    if obj[k]["group"] == True:
                        ...
                        item["name"] = k
                        item["part-key"] = (
                            True if (lrecl + obj[k]["bytes"]) <= partklen else False
                        )
                        item["sort-key"] = (
                            True
                            if (lrecl + obj[k]["bytes"]) <= (sortklen + partklen)
                            and (lrecl + obj[k]["bytes"]) > partklen
                            else False
                        )
                        transf.append(item)
...
if "-copybook" not in iparm or "-output" not in iparm:
    print(
        "Error! The basic sintax is: python3 parse_copybook_to_json.py -copybook <copybookfile.cpb> -output <jsonfile.json>\n"
    )
    quit()
