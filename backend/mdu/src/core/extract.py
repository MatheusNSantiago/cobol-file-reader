...
def FileProcess(log, ExtArgs):
    ...
    if fMetaData.inputtype == "local":
        ...
        inp_temp = (
            fMetaData.general["working_folder"]
            + fMetaData.general["input"].split("/")[-1]
        )
        ...
        if fMetaData.inputtype == "s3":
            ...
            with open(inp_temp, "wb") as f:
                boto3.client("s3").download_fileobj(
                    fMetaData.general["input_s3"], fMetaData.general["input"], f
                )
        ...
        if fMetaData.general["output_type"] in ["file", "s3_obj", "s3"]:
            ...
            log.Write(
                [
                    ...
                    fMetaData.general["working_folder"],
                    fMetaData.general["output"],
                ]
            )

            folder = Path(
                fMetaData.general["working_folder"] + fMetaData.general["output"]
            ).parent

            Path(folder).mkdir(parents=True, exist_ok=True)

            outfile = open(
                fMetaData.general["working_folder"] + fMetaData.general["output"], "w"
            )
            newl = ""
        else:
            outfile = []
    ...
    # Create threads if multi threaded
        ...
        for f in range(1, fMetaData.general["threads"] + 1):
            ...
            p = mp.Process(
                target=queue_worker,
                args=(log, fMetaData, strOutFile, dctQueue[strOutFile], "." + str(f)),
            )
    ...
    while i < fMetaData.general["max"] or fMetaData.general["max"] == 0:
        ...
        record = read(
            InpDS, fMetaData.general["input_recfm"], fMetaData.general["input_recl"]
        )
        ...
        if i > fMetaData.general["skip"]:
            ...
            if fMetaData.general["print"] != 0 and i % fMetaData.general["print"] == 0:
                log.Write(["Records read", str(i)])
    ...
    if layout != "discard":
        ...
        for transf in fMetaData.general[layout]:
            addField(
                ...
                unpack(
                    record[transf["offset"] : transf["offset"] + transf["bytes"]],
                    transf["type"],
                    transf["dplaces"],
                    fMetaData.general["rem_low_values"],
                    False,
                ),
                False,
            )
    ...
    if fMetaData.general["output_type"] in ["file", "s3_obj", "s3"]:
        ...
        if fMetaData.general["output_s3"] != "":
            ...
            log.Write(
                [
                    "Uploading to s3",
                    fMetaData.general["output_s3"],
                    fMetaData.general["output"] + strSuff,
                ]
            )

            if fMetaData.general["verbose"]:
                log.Write(["Source file", OutDs])
            ...
            with open(OutDs, "rb") as f:
                boto3.client("s3").write_get_object_response(
                    Body=f,
                    RequestRoute=fMetaData.general["input_s3_route"],
                    RequestToken=fMetaData.general["input_s3_token"],
                )
