...
type UseGetTableDataProps = {
  group: string;
  file: string;
  copybook: string;
};

export const useGetTableData = ({ group, file, copybook }: UseGetTableDataProps) => {
    ...
    const messageListener = (event: any) => {
      ...
      switch (message.command) {
        case "tableDataReady":
          ...
          console.log(columns);
          console.log(rows);
          // const formattedColumns: MRT_ColumnDef<TableData>[] = columns.map((column: string) => ({
          //   header: column,
          //   accessorKey: column,
          ...
          //   columns.reduce(
          //     (record: TableData, column: string, j: number) => ({ ...record, [column]: row[j] }),
          //     {}
          //   )
          // );
